	...
	public static int pairLumberGarden(RobotController rc, RobotType type){
		try{
			if(type == RobotType.GARDENER){
				if(rc.readBroadcast(LJ_PAIRED) == 2){
					rc.broadcast(LJ_PAIRED, 0);
					return rc.readBroadcast(PAIRING_ID);
				}else{
					rc.broadcast(PAIRING_ID, rc.getID());
					rc.broadcast(LJ_PAIRED, 1);
				}
			}else if(type == RobotType.LUMBERJACK){
				if(rc.readBroadcast(LJ_PAIRED) == 1){
					int id = rc.readBroadcast(PAIRING_ID);
					rc.broadcast(PAIRING_ID, rc.getID());
					rc.broadcast(LJ_PAIRED, 2);
					return id;
				}
			}else{
				System.out.println("[ERROR] Invalid Pairing Type");
			}
		}catch(Exception e){
